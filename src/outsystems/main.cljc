(ns outsystems.main
  "Kotodama WASM entrypoint for the Outsystems clean-room actor (L4) — Clojure port.
  ...
  facts (`outsystems.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "outsystems")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"outsystems.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "outsystems-compat" :tier tier :entities entities} 200])
...
(defn start! [] :outsystems-compat/ready)
